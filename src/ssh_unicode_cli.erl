...
iolist_has_content("") ->
    false;
iolist_has_content(B) when is_binary(B) ->
    true;
iolist_has_content(I) when is_integer(I) ->
    true;
iolist_has_content(L) when is_list(L) ->
    lists:any(fun iolist_has_content/1, L).

%% %%% write out characters
%% %%% make sure that there is data to send
%% %%% before calling ssh_connection:send
write_chars(ConnectionHandler, ChannelId, Chars) ->
    case iolist_has_content(Chars) of
    false ->
      ok;
    true ->
