defmodule Exuvia.Daemon do
  ...
  def init(_) do
    ...
    Logger.info fn ->
      ["listening on ssh://",
       bindspec.host, ":", inspect(bindspec.port),
       ", with a ",
       :blue, inspect(elem(pkb, 0)), :reset,
       " PKI backend"]
       |> IO.ANSI.format
    end
  ...
  defp parse_userinfo(%URI{scheme: scheme, userinfo: userinfo}) do
    userinfo_parts = case userinfo do
      nil -> []
      ""  -> []
    ...
    auth_opts = case {scheme, userinfo_parts} do
      {"ssh", []} -> [
      ...
      {"ssh", ["*", password]} -> [password: String.to_charlist(password)]
      {"ssh", [user, password]} -> [user_passwords: [{String.to_charlist(user), String.to_charlist(password)}]]

      {"github+ssh", [orgs, token]} when is_binary(orgs) and is_binary(token) -> [
        publickey_backend: {Exuvia.KeyBag.Github, [
    ...
    auth_methods_enabled = case MapSet.disjoint?(auth_opts_keys, @password_auth_opts) do
      true  -> 'publickey'
